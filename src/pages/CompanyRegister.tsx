...
import { Eye, EyeOff, Building, Check } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.success('Unternehmens-Account erfolgreich erstellt! Bitte prüfen Sie Ihre E-Mails.');
        navigate('/login');
      }
    } catch (error) {
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center space-y-2">
          <div className="w-12 h-12 mx-auto rounded-xl bg-primary/10 flex items-center justify-center">
            <Building size={24} style={{ color: 'hsl(var(--color-primary))' }} />
          </div>
          <CardTitle className="text-2xl font-heading" style={{ color: 'hsl(var(--foreground))' }}>
            Unternehmen registrieren
          </CardTitle>
          <CardDescription className="text-base">
            Erstellen Sie Ihren Unternehmens-Account
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {isLoading ? 'Wird erstellt...' : 'Unternehmens-Account erstellen'}
              <Building size={16} />
            </Button>
          </form>
